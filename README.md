...
잉크 그라파이트 바탕에 명조 표제. 모든 토큰은 `lib/app.css` 한 곳에 있습니다.

### 강조색을 쓰지 않습니다

예전 `--brand`는 `#5b7cfa`였는데 하락색 `--neg #5b8dee`와 **ΔE 6.1**로 사실상 같은 색이었습니다 — 버튼·포커스·활성 내비가 "하락"과 구별되지 않았습니다. 지금은 조작 가능한 것을 전부 무채색 본지(`--action #f0ede4`)로 표시하고, **색을 가진 것은 상승·하락과 구성 6색뿐입니다.** 이 화면에서 색은 한 가지만 뜻합니다.

`--brand`·`--brand-soft`·`--brand-ink`는 사용처가 많아 이름만 별칭으로 남겨 두었고, 값은 위 무채색을 가리킵니다.

### 활자 세 역할

| 역할 | 페이스 | 쓰는 곳 |
|---|---|---|
| 디스플레이 | Hahmlet 500·600 | 총자산 금액, 표제(`h1`·`h2`·`h3`) |
| 본문 | Pretendard Variable | 레이블·설명·표 본문 |
| 유틸리티 | IBM Plex Mono | 기준일·관측 기간·종목코드(`.stamp`) |

명조를 고른 이유는 통장·결산서·한국은행 통계표가 명조로 조판되기 때문입니다. 이 앱은 "모든 값이 기준일을 달고 다니는 기록"이라는 규칙을 이미 코드에 박아 둔 터라, 그 성격에 맞는 활자가 명조입니다.

**폰트는 `@import`로 받지 않습니다.** 처음엔 그렇게 했는데 `@import`가 렌더를 막아 첫 페인트가 폰트 왕복만큼 밀렸고, 그 지연 때문에 "시세 도착 전에는 금액을 내지 않는다"는 회귀 테스트가 **로딩 구간을 통째로 놓치고 통과**했습니다. 각 화면 `<head>`에서 `media="print"` 뒤바꾸기로 비차단 로드합니다. 폴백 체인은 반드시 유지하세요 — CDN이 막혀도 읽히기만 하면 됩니다.

### 시그니처: 기준일 원장 머리

총자산 숫자는 명조로 조판하고 **억·만·원 단위 글자를 한 단계 작게·흐리게 내려 앉힙니다**(`.total-amount .u`). 자릿수를 세지 않아도 규모가 먼저 읽힙니다. 이 조판은 한국어 금액 표기에만 있는 것입니다.

그 위를 **기준일 레일**(`.rail`)이 지납니다. 이 화면을 지배하는 날짜를 왼쪽 끝에 두고, 기준일이 다른 값이 있으면 눈금으로 찍습니다(가상자산·경제지표). 앱은 이미 날짜가 섞인 데이터를 한 시점의 스냅샷처럼 보여주지 않는데, 그 규칙을 문장이 아니라 구조로 드러내는 줄입니다. **경고가 아닙니다** — 3개월 전 관측은 낡은 값이 아니라 그 시점의 값이고, 정말로 갱신이 필요한 상태는 각 카드가 따로 말합니다.

시그니처는 **홈에만** 둡니다. 모든 화면에 깔면 기억될 것이 없어집니다. 자산 화면은 머릿말의 기준일을 같은 유틸리티 활자(`.stamp`)로만 찍어 가족임을 드러냅니다.

### 카드와 격자

카드는 떠 있는 판이 아니라 놓인 면입니다. 그림자를 걷고 윗면에만 헤어라인(`--edge`)을 남겼습니다 — 이 화면의 구조재는 그림자가 아니라 선입니다.

**격자 칸에는 `min-width:0`이 반드시 있어야 합니다.** 없으면 열이 8개인 보유 자산 표가 칸을 밀어내 선언한 열 비율이 무너집니다(실측 404/299/377px로 찌그러져 있었고, 고친 뒤 435/322/322px로 1.35:1:1을 회복). 좁은 폭에서 페이지 전체가 가로로 310px 밀리던 문제도 같은 원인이었습니다 — 지금은 표가 자기 안에서 가로로 스크롤합니다.

### 모션

하나뿐입니다. 시세가 도착해 숫자가 확정되면 기준일 레일이 왼쪽에서 오른쪽으로 200ms 동안 그어집니다. **숫자 자체는 절대 애니메이션하지 않습니다** — 확정 전 금액을 보여주지 않는다는 규칙과 충돌합니다. `prefers-reduced-motion`에서는 즉시 그려집니다.

### 자산 입력 화면의 토큰 중복을 걷어냈습니다

`asset-input.html`은 색·간격·활자 토큰을 통째로 복사해 갖고 있었고, 그 복사본에는 예전 라이트 테마에서 넘어온 잔재가 남아 있었습니다 — 다크 배경에서 거의 안 보이는 회색 글자 세 개(`#343a3f`·`#4e5358`·`#555b61`), 쓰이지 않는 초록 포커스 글로우(`rgba(23,107,90,.1)`), 그리고 어두운 배경에 어두운 글자를 얹힌 **보이지 않는 토스트**입니다. 지금은 `lib/app.css`를 불러 쓰고, 입력 흐름 전용 변수(`--control-h`·`--cta-h`·`--sheet-max` 등)만 그 화면에 남겨 둡니다. 인라인 `<style>`이 `<link>` 뒤에 오므로 흐름 전용 규칙이 공통 규칙을 이깁니다 — 순서를 바꾸지 마세요.
