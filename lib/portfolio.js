...
(function (global) {
  ...
  function summarize(assets) {
    ...
    assets.forEach(function (asset) {
      ...
      total += valued.krw;
      // 단가 개념이 있는 자산군만 현재가·수량을 낸다. 예적금 잔액이나 부동산 추정가에
      // 억지로 단가를 붙이면 없는 개념을 만들어내는 셈이라 null로 둔다.
      var fields = asset.fields || {};
      var hasUnit = ["equity", "crypto", "commodity"].indexOf(asset.group) >= 0;
      // 원자재 단가는 g당이므로 수량도 g로 맞춘다(금 실물은 돈 단위로 입력받는다).
      var quantity = !hasUnit ? null : asset.group === "commodity" ? Valuation.gramsOf(fields) : Number(fields.quantity || 0);
      var unitPrice = hasUnit ? Valuation.priceFor(asset.group, fields.productCode, fields) : null;
      var unitLabel = asset.group === "commodity" ? "g" : asset.group === "equity" ? "주" : asset.group === "crypto" ? "개" : null;
      rows.push({
        asset: asset,
        id: asset.id,
        group: asset.group,
        ...
        quantity: quantity,
        unitPrice: unitPrice,
        // 단위를 함께 들고 다녀야 화면에서 g·주·개를 혼동하지 않는다.
        unitLabel: unitLabel,
        averagePrice: hasUnit && Number(fields.averagePrice) > 0 ? Number(fields.averagePrice) : null,
