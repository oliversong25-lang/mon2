...
(function (global) {
  ...
  function previousPriceFor(group, code, fields) {
    ...
    // 가상자산은 코인게코가 전일 종가를 주지 않는다. 환율도 전일 값을 싣지 않는다.
    return null;
  }
  ...
  function valuatePrevious(asset) {
    ...
    // 외화는 전일 환율이 없어 원화 환산값을 되돌릴 수 없다.
    if (currency !== "KRW") return unavailable("전일 환율 없음(" + currency + ")");

    // 시세와 무관한 자산은 전일에도 같은 금액이다 — 가격이 움직인 게 아니라 그대로다.
    if (group === "cash" || group === "savings" || group === "fund" || group === "bond" || group === "realestate") {
    ...
    if (group === "equity" || group === "crypto") {
      ...
      if (!price) return unavailable("전일 종가 없음");
      return { krw: Math.round(quantity * price), unavailable: false, reason: null };
    }
