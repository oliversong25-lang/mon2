...
// 실제로 동작하는 것은 홈·자산·경제지표·분석·설정이다 — 나머지는 "준비 중"으로 표시하고
// 누를 수 없게 한다.
...
(function (global) {
  ...
  var ITEMS = [
    ...
    { id: "indicators", label: "경제지표", icon: "◈", href: "indicators.html" },
    { id: "analysis", label: "분석", icon: "◪", href: "analysis.html" },
    // 원칙이 먼저다. 매도 조건은 원칙에서 나오고, 틀이 없는 상태에서 "언제 팔 것인가"만
    // 물으면 임의로 적게 된다 — 그래서 목록에서도 원칙이 기록보다 위다.
    { id: "philosophy", label: "투자 원칙", icon: "◇", href: "philosophy.html" },
    { id: "journal", label: "의사결정 기록", icon: "✎", href: "decisions.html" },
  ...
  function nav(activeId, options) {
    ...
    return (
      ...
      '<div class="nav-foot">' +
      '<span class="nav-mark">내</span><span class="nav-profile-label">내 자산</span>' +
      '<button type="button" class="nav-logout" data-nav-logout aria-label="로그아웃" title="로그아웃">' +
  ...
  function mount(rootId, activeId, innerHtml, options) {
    ...
    var logout = root.querySelector("[data-nav-logout]");
    if (logout) {
      logout.addEventListener("click", async function () {
        // 서버 로그아웃이 끝나는 동안 여러 번 요청하지 않도록 버튼을 즉시 잠근다.
        logout.disabled = true;
        logout.setAttribute("aria-busy", "true");
        if (global.AccountStore && typeof global.AccountStore.signOut === "function") {
          await global.AccountStore.signOut();
        } else {
          // 셸만 단독으로 불러온 예외 상황에서도 보호 화면에 머물지 않게 한다.
