...
(function (global) {
  ...
  // 어느 변형의 숫자인가. 버전 문자열만 보고 v1.1이라고 읽는 일이 없게 따로 꺼낸다.
  function variant() {
    return (state.status === "ready" && state.data.variant) || null;
  }

  // 성숙도. **검증된 국면이 아니면 신호를 내주지 않는다** — 화면마다 범위를 다시
  // 판정하게 두면 한쪽에서 조용히 새기 때문에, 거르는 자리를 여기 하나로 둔다.
  function maturity() {
    if (state.status !== "ready") return null;
    var data = state.data.maturity;
    if (!data || !data.current || !data.current.validated) return null;
    return data;
  }

  // 검증 범위는 신호가 없을 때도 보여야 한다. 왜 이 국면에는 문구가 없는지가
  // 그 표에만 적혀 있기 때문이다.
  function maturityScope() {
    return (state.status === "ready" && state.data.maturity) || null;
  }

  function varianceDistribution() {
    return (state.status === "ready" && state.data.varianceDistribution) || null;
  }
