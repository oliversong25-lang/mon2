...
(function (global) {
  ...
  // 화면은 언제나 로컬 사본부터 갱신한다. 네트워크 저장은 AccountStore가 맡기 때문에
  // 연결이 끊겨도 사용자가 방금 입력한 값은 이 기기에서 사라지지 않는다.
  function write(value) {
    try {
      localStorage.setItem(KEY, JSON.stringify(value));
    } catch (error) {
      console.error("[세션] 로컬 저장 실패:", error);
      if (global.AccountStore) global.AccountStore.showStorageError();
      return false;
    }
    if (global.AccountStore) global.AccountStore.queueSave(value);
    return true;
  }
