...
def build_payload(settings: Any) -> dict[str, Any]:
    ...
    payload["verdict"] = {
        ...
        "statement": (
            ...
            "평탄역 **아래**임을 보이고, C는 확장 역사에서 17주가 명목 유의에 도달함을 "
            "보인다. 권고를 persist13w에서 **persist17w**로 조정한다. "
            "(C의 유의성은 다음 단계 `boundary_reading`이 보정한다 — 격자를 쓸어 고른 "
            "최댓값이라 보정하면 문턱을 넘지 못하고, 유의성은 1992년 이후 구간에만 "
            "있다. 권고는 그래도 B가 홀로 받친다.)"
            if payload["a"]["passes"]
